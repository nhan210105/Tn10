    ...
    static func apply(
        ...
        do {
            ...
            for (index, resolved) in resolvedRules.enumerated() {
                ...
                } catch let error as PatchApplyDiagnosticError {
                    throw error
                } catch {
                ...
                do {
                    guard try digestFile(resolved.target) == records[index].replacementDigest else {
                        throw PatchApplyDiagnosticError(
                            ...
                            bundleID: resolved.rule.bundleID,
                            path: resolved.target.path,
                            detail: "SHA-256 của file sau khi ghi không trùng replacement data."
                        )
                    }
                    log("patch: VERIFY ok bundle=\(resolved.rule.bundleID) path=\(resolved.target.path)")
                } catch let error as PatchApplyDiagnosticError {
                    throw error
                } catch {
                    throw PatchApplyDiagnosticError(
            ...
            do {
                ...
                if let diagnostic = failure as? PatchApplyDiagnosticError {
                    throw PatchApplyDiagnosticError(
                        stage: .rollback,
                        bundleID: diagnostic.bundleID,
                        path: diagnostic.path,
                        detail: "\(diagnostic.detail) | Rollback cũng lỗi: \(rollbackError.localizedDescription)"
                    )
                }
                throw PatchApplyDiagnosticError(
                    stage: .rollback,
                    bundleID: resolvedRules.first?.rule.bundleID ?? "unknown",
                    path: resolvedRules.first?.target.path ?? "<unknown>",
                    detail: "\(failure.localizedDescription) | Rollback cũng lỗi: \(rollbackError.localizedDescription)"
                )
            }
            if let diagnostic = failure as? PatchApplyDiagnosticError {
                throw diagnostic
    ...
    private static func atomicWrite(
        ...
        guard fileManager.createFile(atPath: staging.path, contents: data, attributes: attributes) else {
            throw PatchApplyDiagnosticError(
                ...
                bundleID: bundleID,
                path: target.path,
                detail: "Không tạo được staging file \(staging.path). errno=\(errno)"
            )
        }
        ...
        guard rename(staging.path, target.path) == 0 else {
            throw PatchApplyDiagnosticError(
                ...
                bundleID: bundleID,
                path: target.path,
                detail: "rename staging -> target thất bại. errno=\(errno)"
            )
        }
    }
