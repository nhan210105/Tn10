...
enum ContainerStore {
    ...
    // iOS 26.x can resolve an Application container path while still denying
    // direct filesystem writes to that container. 3105's newer container
    // flow keeps a scoped sandbox grant alive for the lifetime of the access
    // session. Keep the same lifetime semantics here instead of resolving a
    // path and immediately losing the access token.
    private static var activeContainerGrants: [String: Int64] = [:]
    private static let containerGrantLock = NSLock()
    ...
    /// Ensures a scoped sandbox extension is consumed and retained for an
    /// Application container. This is the fallback used by 3105 on iOS 26.x
    /// when MCM can resolve the UUID but its normal activation lease is not
    /// sufficient for direct R/W access.
    @discardableResult
    static func grantContainerAccess(_ containerPath: String) -> Int64 {
        ...
        containerGrantLock.lock()
        defer { containerGrantLock.unlock() }

        if let existing = activeContainerGrants[clean], existing >= 0 {
            return existing
        }
        ...
        guard handle >= 0 else {
            log("patch: scoped container grant FAILED path=\(clean) handle=\(handle)")
            return handle
        }

        // Intentionally retain the handle. Releasing it immediately would
        // revoke the extension before PatchTransaction performs WRITE.
        activeContainerGrants[clean] = handle
        log("patch: scoped container grant OK path=\(clean) handle=\(handle)")
        return handle
    }
